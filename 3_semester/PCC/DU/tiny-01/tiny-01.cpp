...
std::vector<std::string> pluralize(const std::vector<std::string>& words) {
    std::vector<std::string> res;
    
    for (const auto& word : words) {
        std::string pluralWord = pluralize(word);
        res.push_back(pluralWord);
    }
    
    return res;
}

// The code modifies the original vector and returns it.